...
async fn process_notification(
    ...
    builder.collapse_key(config.collapse_key);
    builder.data(&push_notification.notification);
    builder.badge(unread_string);
    builder.click_action(config.fcm_notification_click_action);
    builder.data(&map);

    // Set additional keys for the notification message
    if !data_message_mode && !is_clearing_notification {
        let title = str::replace(config.fcm_notification_title.copy(), "count", unread_string);
        builder.title(title);
        builder.body(config.fcm_notification_body);
        builder.badge(unread_string);
        builder.sound(config.fcm_notification_sound);
        builder.icon(config.fcm_notification_icon);
        builder.tag(config.fcm_notification_tag);
        builder.android_channel_id(config.fcm_notification_android_channel_id);
    }

    // Send the fcm message
    let response = fcm_client.send(builder.finalize()).await;
    ...
    if response.ok {
        ...
        if response
            .into_result()
            .iter()
            .any(|response| response.error.ok)
        {
            rejected.append(
                push_notification
                    .notification
        ...
        return Ok(HttpResponse::Ok().json(PushGatewayResponse { rejected: rejected }));
    } else {
        return Err(MatrixError {
            error: String::from("Invalid response from upstream push service"),
            errcode: ErrCode::M_UNKNOWN,
        });
    }
}
